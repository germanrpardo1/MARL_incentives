...
def addGenericOptions(argParser):
    ...
    argParser.add_argument(
        ...
        dest="net",
        category="input",
        type=argParser.net_file,
        help="SUMO network (mandatory)",
        metavar="FILE",
    )
    argParser.add_argument(
        "-+",
        ...
        category="input",
        default="",
        type=argParser.additional_file,
    ...
    argParser.add_argument(
        ...
        type=argParser.time,
        default=0,
        help="Set simulation/routing begin",
    )
    argParser.add_argument(
        "-e", "--end", type=argParser.time, help="Set simulation/routing end"
    )
    argParser.add_argument(
        "-R", "--route-steps", type=int, default=200, help="Set simulation route steps"
    )
    argParser.add_argument(
        "-a",
        "--aggregation",
        type=argParser.time,
        default=900,
        help="Set main weights aggregation period",
    )
    argParser.add_argument(
        "-m",
        "--mesosim",
        action="store_true",
        default=False,
        help="Whether mesosim shall be used",
    )
    ...
    argParser.add_argument(
        ...
        action="store_true",
        default=False,
        help="use current time to generate random number",
    )
    argParser.add_argument(
        "-I",
        "--nointernal-link",
        ...
        dest="internallink",
        default=False,
        help="not to simulate internal link: true or false",
    )
    argParser.add_argument(
        "-j",
        "--meso-junctioncontrol",
        action="store_true",
        default=False,
    ...
    argParser.add_argument(
        ...
        action="store_true",
        default=False,
        help="eager insertion tests (may slow down the sim considerably)",
    )
    argParser.add_argument(
        "--time-to-teleport",
        dest="timetoteleport",
    ...
    argParser.add_argument(
        ...
        action="store_true",
        default=False,
        help="zip old iterations using 7zip",
    )
    argParser.add_argument(
        "-s",
        ...
        dest="MSA",
        default=False,
        help="apply the method of successive average as the swapping algorithm",
    )
...
def initOptions():
    ...
    argParser.add_argument(
        "--continue-on-unbuild",
        action="store_true",
        dest="continueOnUnbuild",
        default=False,
        help="continues on unbuild routes",
    )
    argParser.add_argument(
        "-t",
        "--trips",
        help="trips in step 0 (either trips, flows, or routes have to be supplied)",
        metavar="FILE",
    )
    argParser.add_argument(
        "-r",
        "--routes",
        help="routes in step 0 (either trips, flows, or routes have to be supplied)",
        metavar="FILE",
    )
    argParser.add_argument(
        "-F",
        "--flows",
        help="flows in step 0 (either trips, flows, or routes have to be supplied)",
        metavar="FILE",
    )
    argParser.add_argument(
        "-A", "--gA", type=float, default=0.5, help="Sets Gawron's Alpha"
    )
    argParser.add_argument(
        "-B", "--gBeta", type=float, default=0.9, help="Sets Gawron's Beta"
    )
    argParser.add_argument(
        "-E",
        "--disable-summary",
        ...
        dest="noSummary",
        default=False,
        help="No summaries are written by the simulation",
    )
    argParser.add_argument(
        "-T",
        "--disable-tripinfos",
        ...
        dest="noTripinfo",
        default=False,
        help="No tripinfos are written by the simulation",
    )
    argParser.add_argument(
        "--tripinfo-filter", dest="tripinfoFilter", help="filter tripinfo attributes"
    )
    argParser.add_argument(
        "--inc-start",
        dest="incStart",
        type=float,
        default=0,
        help="Start for incrementing scale",
    )
    argParser.add_argument(
        "--inc-max",
        dest="incMax",
        type=float,
        default=1,
        help="Maximum for incrementing scale",
    )
    argParser.add_argument(
        "--inc-base",
        dest="incBase",
    ...
    argParser.add_argument(
        ...
        type=int,
        default=0,
        help="Give the time incrementation",
    )
    argParser.add_argument(
        "-f",
        "--first-step",
    ...
    argParser.add_argument(
        "-l", "--last-step", dest="lastStep", type=int, default=50, help="Last DUA step"
    )
    argParser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of iterations to use for convergence calculation",
    )
    argParser.add_argument(
        "--max-convergence-deviation",
        dest="convDev",
        type=float,
        help="Maximum relative standard deviation in travel times",
    )
    argParser.add_argument(
    ...
    argParser.add_argument(
        ...
        default="None",
        help="choose the format of the route file",
    )
    argParser.add_argument(
        "-z",
        "--output-lastRoute",
    ...
    argParser.add_argument(
        ...
        dest="allroutes",
        default=False,
        help="save routes with near zero probability",
    )
    argParser.add_argument(
    ...
    argParser.add_argument(
        "--skip-first-routing",
        action="store_true",
        dest="skipFirstRouting",
        default=False,
        help="run simulation with demands before first routing",
    )
    argParser.add_argument(
        "--logit",
        action="store_true",
        default=False,
        help="use the logit model for route choice",
    )
    argParser.add_argument(
        "-g",
        "--logitbeta",
        type=float,
        default=0.15,
        help="use the c-logit model for route choice; logit model when beta = 0",
    )
    argParser.add_argument(
        "-i",
        "--logitgamma",
        type=float,
        default=1.0,
        help="use the c-logit model for route choice",
    )
    argParser.add_argument(
        "-G", "--logittheta", type=float, help="parameter to adapt the cost unit"
    )
    argParser.add_argument(
        "-J", "--addweights", help="Additional weights for duarouter"
    )
    argParser.add_argument(
        "--convergence-steps",
        dest="convergenceSteps",
        type=int,
    ...
    argParser.add_argument(
        ...
        action="store_true",
        default=False,
        help="use added weights only on the first iteration",
    )
    argParser.add_argument(
        "--router-verbose",
        action="store_true",
        default=False,
        help="let duarouter print some statistics",
    )
    argParser.add_argument(
        "--weight-memory",
        action="store_true",
    ...
    argParser.add_argument(
        ...
        dest="clean_alt",
        default=False,
        help="Whether old rou.alt.xml files shall be removed",
    )
    argParser.add_argument(
        "--binary",
        dest="gzip",
...
def call(command, log):
    ...
    if retCode != 0:
        print(
            ("Execution of %s failed. Look into %s for details.") % (command, log.name),
            file=sys.stderr,
        )
    return retCode


def writeRouteConf(duarouterBinary, step, options, dua_args, file, output, routesInfo):
    ...
    args = [
        ...
        output,
        "--exit-times",
        str(routesInfo == "detailed"),
        "--ignore-errors",
        str(options.continueOnUnbuild),
        "--with-taz",
        str(options.districts is not None),
        "--gawron.beta",
        str(options.gBeta),
        "--gawron.a",
        str(options.gA),
        "--keep-all-routes",
        str(options.allroutes),
        "--routing-algorithm",
        ...
        "--weights.expand",
        "--logit.beta",
        str(options.logitbeta),
        "--logit.gamma",
        str(options.logitgamma),
        "--random",
        str(options.absrand),
        "--begin",
        str(options.begin),
        "--verbose",
        str(options.router_verbose),
        "--no-step-log",
        "--no-warnings",
        str(options.noWarnings),
    ]
...
def get_scale(options, step):
    ...
    if options.incBase > 0:
        return min(
            options.incStart + options.incValue * float(step + 1) / options.incBase,
            options.incMax,
        )
    else:
...
def writeSUMOConf(sumoBinary, step, options, additional_args, route_files):
    ...
    sumoCmd = [
        ...
        options.route_steps,
        "--no-internal-links",
        options.internallink,
        "--eager-insert",
        options.eager_insert,
        "--time-to-teleport",
        options.timetoteleport,
        "--time-to-teleport.highways",
        options.timetoteleport_highways,
        "--verbose",
        "--no-warnings",
        options.noWarnings,
    ] + additional_args
    ...
    if options.mesosim:
        sumoCmd += ["--mesosim", "--meso-recheck", options.meso_recheck]
...
def assign_remaining_args(application, prefix, args):
    ...
    for arg in args:
        if "--" in arg:
            ...
            if item is None:
                sys.exit('Encountered argument "%s" without a preceding option' % arg)
            item.append(arg)
    if item is not None:
        items.append(item)
        ...
        for interval_prv in tree_sumo_prv.getroot():
            ...
            for edge_prv in interval_prv.iter("edge"):
                if edge_prv.get("traveltime") is not None:
                    veh_prv = float(edge_prv.get("left")) + float(
                        edge_prv.get("arrived")
                    )
                ...
                for edge_cur in interval_cur.iter("edge"):
                    if edge_cur.get("traveltime") is not None:
                        ...
                        if id_cur in oldIntervalValues:
                            ...
                            veh_cur = float(edge_cur.get("left")) + float(
                                edge_cur.get("arrived")
                            )
                            tt_cur = float(edge_cur.get("traveltime"))
                            dif_tt = abs(tt_cur - tt_prv)
                            dif_veh = abs(veh_cur - veh_prv)
                            if dif_veh != 0:
                                mc_cur = (dif_tt / dif_veh) * (
                                    veh_cur**options.mcExp
                                ) + tt_cur
                            else:
                            ...
                            if DEBUGLOG:
                                if begin_cur == "1800.00":
                                    print(
                                        "step=%s beg=%s e=%s tt=%s ttprev=%s n=%s nPrev=%s mC=%s mCPrev=%s"
                                        % (
                                            step,
                                            begin_cur,
                                            edgeID,
...
def generateEdgedataAddFile(EDGEDATA_ADD, options):
    ...
    with open(EDGEDATA_ADD, "w") as fd:
        vTypes = (
            ' vTypes="%s"' % " ".join(options.measureVTypes.split(","))
            if options.measureVTypes
            else ""
        )
        print("<a>", file=fd)
        print(
            '    <edgeData id="dump_%s" freq="%s" file="%s" excludeEmpty="true" minSamples="1"%s/>'
            % (
                options.aggregation,
                options.aggregation,
        ...
        if options.eco_measure:
            print(
                (
                    '    <edgeData id="eco_%s" type="emissions" freq="%s" file="%s" '
...
def main(args=None):
    ...
    if (not options.trips and not options.routes and not options.flows) or (
        options.trips and options.routes
    ):
        argParser.error("Either --trips, --flows, or --routes have to be given!")
    ...
    if options.weightmemory:
        costmemory = CostMemory(
            "traveltime", pessimism=options.pessimism, network_file=options.net
        )
    routesSuffix = ".xml"
    if options.gzip:
        routesSuffix = ".gz"
    ...
    for step in range(options.firstStep, options.lastStep):
        ...
        if not (options.skipFirstRouting and step == 0):
            simulation_demands = [
                "%03i/%s_%03i.rou%s" % (step, get_basename(f), step, routesSuffix)
                for f in input_demands
            ]
        if not ((options.skipFirstRouting and step == 1) or step == 0):
            router_demands = [
                "%03i/%s_%03i.rou.alt%s"
                % (step - 1, get_basename(f), step - 1, routesSuffix)
                for f in input_demands
            ]
        ...
        sumocfg = writeSUMOConf(
            sumoBinary, step, options, sumo_args, ",".join(simulation_demands)
        )  # todo: change 'grou.xml'
        log.flush()
        sys.stdout.flush()
        ...
        if options.weightmemory:
            ...
            print(
                ">>> Error avg:%.12g mean:%.12g"
                % (costmemory.avg_error(), costmemory.mean_error())
            )
            print(
                ">>> Absolute Error avg:%.12g mean:%.12g"
        ...
        if options.zip and step - options.firstStep > 1:
            ...
            zipProcesses[zipStep] = subprocess.Popen(
                ["7z", "a", "iteration%03i.7z" % zipStep, "%03i" % zipStep],
                stdout=zipLog,
                stderr=zipLog,
            )
        ...
        if options.convDev:
            ...
            for t in sumolib.output.parse_fast(
                "%03i/tripinfo_%03i.xml" % (step, step), "tripinfo", ["duration"]
            ):
                sum += float(t.duration)
                count += 1
            if count == 0:
                sys.exit(
                    "Error: No vehicle has reached its destination within the simulation duration."
                )
            avgTT.add(sum / count)
