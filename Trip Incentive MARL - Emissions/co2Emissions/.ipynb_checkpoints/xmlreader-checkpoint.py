...
def main(path):
    ...
    for timestep in root.findall('.//timestep'):
        ...
        for vehicle in timestep.findall('.//vehicle'):     # Get the vehicles informations
            ...
            fuel = 'gasoline'                       # comment this one if you have the fuel type in the xml file
            speed = float(vehicle.get('speed'))         # Get the vehicles speed
            acceleration = float(vehicle.get('acceleration'))  # Get the vehicles acceleration
            emission = float(co2modeler(speed,acceleration,type,fuel))   # Calculate the emissions assuming gasoline passenger vehicle

            if vehicle.get('id') not in emissions_per_vehicle: # Create key for each lane
            ...
            subtotal_emission += emission   # Add the emissions to the instantaneous emissions

        total_emissions += subtotal_emission # Add the instantaneous emissions to the total emissions
